...
where $x_1$ represents the mean of observed values, $x_1$, $x_2$, ... $x_n$ represents the n observed values.
---
## Median

If all the observations are listed from smallest to largest (or vice versa), median is the observation that is in the middle. 
...
$$s^2 = \frac{\Sigma_{i = 1}^{n} (x - \bar x )^2}{n-1}$$ 
...
<br>
.formula[
$$s = \sqrt{\frac{\Sigma_{i = 1}^{n} (x - \bar x )^2}{n-1}}$$
]
